...
> **定位说明**：本工程是针对**我们这一台**标线车的特定硬件写的控制实现，不是通用
> 产品固件。引脚分配、步进方向（`StepMotor_New.h` 的 `M1_FWD` ~ `M4_REV`）、加减速
> 斜坡（`RAMP_SOFT_ARR` 等）、舵机角度范围（`Servo.c`，0~180° → 500~2500us）、车位
> 尺寸（`system.c` 的 `PAINT_*_PULSE`）、以及上位机 `SPRAY_INTERVAL_S` 与下位机
> `RESPRAY_PULSE` 的节拍配合 —— 这些**全部按实车标定过**，换一台车、换一套接线、
> 换一组电机都得重新量。
>
> 这是真实的机电设备，上机前请先读下面的 **安全须知**。

## ⚠️ 安全须知

这是一台**会自己动、还会喷漆**的机器。下面几条不是形式主义，是这个项目结构上真实
存在的风险 —— 上车前逐条过一遍。

**1. 上电瞬间舵臂就会动。**
`Hardware_Init()` 一进去就调 `Servo_SetAngle(40)` / `Servo2_SetAngle(90)`，因为
DPWM 初始 `TIM_Pulse = 0`，不写这两句舵机收不到有效信号。**上电前手离舵臂远点。**

**2. 串口协议没有校验，线上的噪声就是指令。**
9600 8N1、单字节 ASCII，没有帧头、没有校验和。串口线上任何一个乱字节（拔插、
上电毛刺、蓝牙丢包）都可能被状态机当成 `U`（前进）甚至 `P`（自动画线）。
**蓝牙那条尤其不可信。** → 第一次跑某条链路时，务必先把轮子架空。

**3. 步进是开环的，车自己在哪它并不知道。**
没有编码器，丢步不可恢复，每次上电的位置 = 上次断电那一刻 + 一个未知误差。所以
`P` 不是"画一个 5.3m × 2.5m 的矩形"，而是"**从车现在的位置**画一个"。要消掉
累积误差，只能加限位 / 回零开关。

**4. `T`（急停）是软件停止，不切电源。**
它让状态机停在 `TING`，但电机使能、泵的 GPIO 都还带电。**手边必须有一个物理
断电手段**（电池开关 / 拔插头），别只靠串口发 `T`。

**5. 泵里先装水，别装漆。**
确认过时序、喷幅、行进速度都对了再换漆。任何时候不要把喷头对着人。

### 首次上车，按这个顺序走

1. **不接电机、不接泵**，只上电 —— 看 OLED 有没有显示、PB15 心跳灯是否 500ms 翻转。
2. **轮子架空、泵接清水** —— 逐条试方向（`U` / `R` / `Y` / `Z` / `A` / `X`），
   确认转向和预期一致；试 `T`，确认松开后确实不动了。
3. **落地但空跑** —— 不接泵，跑一遍 `P`，量实际画出来的尺寸对不对。
4. **最后才接漆**，头几次跑把手放在电源开关上。

调试时**不要绕过梯形斜坡**（`RAMP_SOFT_ARR`），也不要在加速段中间硬改频率 ——
丢步就是这么来的。

本项目按 MIT 提供，作者不对使用后果负责（免责条款见 [LICENSE](LICENSE)）。
**测试安全和现场安全由你自己负责** —— 上面是已知的坑，不是完整清单。
